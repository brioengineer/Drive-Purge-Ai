  ...
  const startAnalysis = async (isDemo: boolean = false) => {
    ...
    if (!isDemo && !isAuthenticated) {
        try {
            ...
            setError({
                title: "Authentication Policy Error",
                msg: e.message || "The Google Login popup failed. This is usually a policy or origin mismatch.",
    ...
    setAgentMessage("Indexing file metadata...");
    ...
    try {
      ...
      setAgentMessage("Analyzing with Gemini AI...");
    ...
    <div className="min-h-screen flex flex-col font-sans bg-white text-slate-900">
      <nav className="glass-panel sticky top-0 z-50 px-8 py-4 flex items-center justify-between border-b border-slate-100">
        ...
        <div className="flex items-center gap-6">
          {isAuthenticated && (
            <div className="px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-[10px] font-black uppercase tracking-widest">Connected</div>
          )}
          {state === AppState.REVIEWING && (
            <button onClick={handleCleanup} disabled={selectedIds.size === 0} className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-30 text-white px-6 py-2 rounded-full font-bold text-xs shadow-lg">
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full p-6 lg:p-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                    <div>
                        <h1 className="text-5xl font-black text-slate-900 leading-[1.1] mb-8">
                            Reclaim space with <br/> <span className="text-indigo-600">AI precision.</span>
                        </h1>
                        <p className="text-lg text-slate-500 mb-10">Intelligent audit for Google Drive.</p>
                        <div className="flex flex-col sm:flex-row gap-4">
                            <button onClick={() => startAnalysis(false)} className="bg-slate-900 text-white px-10 py-5 rounded-2xl font-bold text-lg hover:bg-indigo-600 transition-all flex items-center justify-center gap-3 shadow-xl">
                            ...
                            <button onClick={() => startAnalysis(true)} className="bg-white border-2 border-slate-100 text-slate-500 px-10 py-5 rounded-2xl font-bold text-lg">Try Demo Scan</button>
                        </div>
                    </div>
                </div>
            </div>
          ...
          <div>
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex gap-4">
                  <button onClick={() => setSelectedIds(new Set(candidates.map(c => c.id)))} className="text-[10px] font-black text-indigo-600 uppercase">Select All</button>
                  <button onClick={() => setSelectedIds(new Set())} className="text-[10px] font-black text-slate-300 uppercase">Clear</button>
                </div>
              </div>
        ...
        {state === AppState.COMPLETED && (
            <div className="py-20 text-center">
                ...
                <button onClick={() => setState(AppState.LANDING)} className="bg-slate-900 text-white px-10 py-4 rounded-2xl font-bold">Return to Dashboard</button>
            </div>
        )}

        {error && (
            <div className="mt-12 p-8 bg-rose-50 border border-rose-100 rounded-[24px] flex flex-col gap-6 animate-in slide-in-from-top-4">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                      ...
                      <p className="text-sm text-rose-700 leading-relaxed mt-1">
                        Google rejected the authentication request. Follow these steps to fix the "doesn't comply" error:
                      </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-white p-6 rounded-xl border border-rose-200 shadow-sm">
                    <h5 className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-4">Step 1: Authorized Origins</h5>
                    <p className="text-xs text-slate-600 mb-4">In Google Cloud Console, ensure your <strong>Authorized JavaScript Origin</strong> is exactly:</p>
                    <code className="block bg-slate-100 p-2 rounded font-mono text-xs font-bold text-slate-800 mb-2 select-all">https://brioengineer.github.io</code>
                    <p className="text-[10px] text-rose-500 font-bold italic">Note: Remove any trailing slashes or subfolders.</p>
                  </div>

                  <div className="bg-white p-6 rounded-xl border border-rose-200 shadow-sm">
                    <h5 className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-4">Step 2: Add Test User</h5>
                    <p className="text-xs text-slate-600 mb-2">If your app status is <strong>"Testing"</strong> (not Verified):</p>
                    <ol className="text-xs text-slate-600 space-y-2 list-decimal pl-4">
                      <li>Go to <strong>OAuth Consent Screen</strong> tab.</li>
                      <li>Find the <strong>Test Users</strong> section at the bottom.</li>
                      <li>Click <strong>+ Add Users</strong> and add your email.</li>
                    </ol>
                  </div>
                </div>
                
                <div className="bg-amber-50 p-4 rounded-lg border border-amber-200 text-xs text-amber-800 italic">
                  * After updating settings in the Google Cloud Console, wait 5 minutes for Google's servers to sync before trying again.
                </div>
                
                <button onClick={() => setError(null)} className="self-center px-8 py-2 bg-rose-600 text-white rounded-full text-xs font-bold">I've updated the settings, close this error</button>
            </div>
        )}
      </main>
    </div>
